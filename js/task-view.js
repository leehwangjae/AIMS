...
function renderCommentForm() {
  return `
    <form id="taskCommentForm" class="form-grid">
      ...
      <label class="form-field full"><span>코멘트</span><textarea id="${TASK_COMMENT_TEXT_ID}" name="comment" rows="3" placeholder="팀장/참여교원 코멘트 또는 담당자 답변을 입력하세요."></textarea></label>
      <div class="form-actions"><button class="btn btn-primary" type="submit">코멘트 등록</button></div>
    </form>`;
}
...
function bindCommentForm() {
  ...
  form.addEventListener('submit', event => {
    ...
    upsertItem('taskComments', {
      id: `task_comment_${Date.now()}`,
...
function renderTaskSummary() {
  ...
  target.innerHTML = `
    <div class="kpi-card-grid">
      <div class="metric-card"><div class="metric-value">${tasks.length}</div><div class="metric-label">전체 업무</div></div>
      ${STATUSES.map(status => `<div class="metric-card"><div class="metric-value">${tasks.filter(task => task.status === status.id).length}</div><div class="metric-label">${status.label}</div></div>`).join('')}
      <div class="metric-card"><div class="metric-value">${delayed}</div><div class="metric-label">지연 업무</div></div>
    </div>`;
}
...
function renderTaskBoard() {
  ...
  target.innerHTML = `
    <div class="kanban-board">
      ${STATUSES.map(status => renderKanbanColumn(status, tasks.filter(task => task.status === status.id))).join('')}
    </div>`;

  target.querySelectorAll('[data-task-status]').forEach(button => {
    button.addEventListener('click', () => changeTaskStatus(button.dataset.taskStatus, button.dataset.nextStatus));
  });
...
function renderKanbanColumn(status, tasks) {
  return `
    <div class="kanban-column">
      <div class="kanban-column-head"><strong>${status.label}</strong><span>${tasks.length}</span></div>
      <div class="kanban-card-list">
        ${tasks.length ? tasks.map(renderTaskCard).join('') : '<div class="kanban-empty">해당 업무 없음</div>'}
      </div>
    </div>`;
}
...
function renderTaskCard(task) {
  ...
  return `
    <article class="kanban-card priority-${task.priority}">
      <div class="kanban-card-top"><span class="priority-badge">${getPriorityLabel(task.priority)}</span><span class="due-badge ${isDelayed(task) ? 'delayed' : ''}">${getDueText(task)}</span></div>
      <h4>${task.title}</h4>
      <div class="kanban-meta">${getUnitName(task.unitTaskId)} · ${task.type || '일반업무'}</div>
      <div class="kanban-meta">담당자: <strong>${task.owner}</strong></div>
      <div class="progress" style="margin:8px 0;"><div class="progress-bar" style="width:${Math.min(Number(task.progress || 0), 100)}%"></div></div>
      <div class="kanban-meta">진행률 ${task.progress || 0}%</div>
      ${task.description ? `<p class="kanban-desc">${task.description}</p>` : ''}
      ${task.issue ? `<p class="kanban-issue">요청/애로: ${task.issue}</p>` : ''}
      ${comments.length ? `<div class="kanban-comments">${comments.slice(-2).map(comment => `<div><strong>${comment.author}</strong> ${comment.comment}</div>`).join('')}</div>` : ''}
      <div class="kanban-actions">
        ${STATUSES.filter(status => status.id !== task.status).map(status => `<button class="btn btn-outline" type="button" data-task-status="${task.id}" data-next-status="${status.id}">${status.label}</button>`).join('')}
        <button class="btn btn-outline" type="button" data-delete-task="${task.id}">삭제</button>
      </div>
    </article>`;
}
...
function updateCommentTargetOptions() {
  ...
  if (!select) return;
  const tasks = getCollection('tasks');
  select.innerHTML = tasks.length ? tasks.map(task => `<option value="${task.id}">${task.title} / ${task.owner}</option>`).join('') : '<option value="">등록된 업무 없음</option>';
}
...
function seedTasksIfEmpty() {
  ...
  [
    ...
    { title: '스마트모빌리티 실습 프로그램 기획', unitTaskId: '1-2', owner: '담당자', dueDate: '2026-07-05', status: 'TODO', progress: 10, priority: 'NORMAL', type: '프로그램기획', description: '실습장비 및 기업 연계 프로그램 구성', issue: '' },
...
function getUnitName(unitTaskId) {
  return UNIT_OPTIONS.find(unit => unit.id === unitTaskId)?.name || unitTaskId;
}

function getPriorityLabel(priority) {
  return PRIORITIES.find(item => item.id === priority)?.label || priority || '일반';
}
...
function getDueText(task) {
  if (!task.dueDate) return '마감 미정';
  if (task.status === 'DONE') return `완료`;
  const today = new Date(new Date().toISOString().slice(0, 10));
  const due = new Date(task.dueDate);
  const diff = Math.ceil((due - today) / (1000 * 60 * 60 * 24));
  if (diff < 0) return `기한초과 ${Math.abs(diff)}일`;
