...
function bindPlanDraftForm() {
  ...
  form.querySelector(`#${KPI_SELECT_ID}`)?.addEventListener('change', () => {
    updateKpiPerformanceOptions();
    updateExpectedKpiResult();
  });
  form.querySelector(`#${KPI_TYPE_ID}`)?.addEventListener('change', () => {
...
function getKpiPerformanceOptions(kpiName) {
  if (kpiName.includes('인력양성')) {
    return [
      { label: '학부생', guide: '학부생 1명 = 1.0명 인정', weight: 1 },
      { label: '석사', guide: '석사 1명 = 1.5명 인정', weight: 1.5 },
      { label: '박사', guide: '박사 1명 = 2.0명 인정', weight: 2 },
      { label: '나노디그리', guide: '나노디그리 1명 = 0.3명 인정', weight: 0.3 }
    ];
  }

  if (kpiName === '취업률') {
    return [
      { label: '지역내 취업', guide: '지역내 취업 1명 = 1.0명 인정', weight: 1 },
      { label: '지역외 취업', guide: '지역외 취업 1명 = 0.5명 인정', weight: 0.5 }
    ];
  }

  if (kpiName.includes('캡스톤')) {
    return [
      { label: '일반', guide: '일반 캡스톤 1건 = 1.0건 인정', weight: 1 },
      { label: '기업연계형', guide: '기업연계형 캡스톤 1건 = 1.5건 인정', weight: 1.5 },
      { label: '융합형', guide: '융합형 캡스톤 1건 = 2.0건 인정', weight: 2 },
      { label: '글로벌', guide: '글로벌 캡스톤 1건 = 2.0건 인정', weight: 2 }
    ];
  }

  if (kpiName === '산학협력지수') {
    return [
      { label: 'A 교과목 개편', guide: '산업체 수요 기반 교과목 개편 수 / 5개년 목표 40', weight: 1 },
      { label: 'B 기업참여형 캡스톤', guide: '기업멘토 참여 캡스톤 과제 수 / 연간 목표 23', weight: 1 },
      { label: 'C 협력기업 MOU', guide: '전략산업분야 기업 MOU 누적 건수 / 5개년 목표 30', weight: 1 },
      { label: 'D 재직자 교육과정', guide: '재직자 재교육 프로그램 누적 운영 횟수 / 5개년 목표 30', weight: 1 }
    ];
  }
  ...
  if (kpiName.includes('단기전문교육과정')) {
    return [
      { label: '교육과정 개설', guide: '단기전문교육과정 1개 과정 = 1건 인정', weight: 1 },
      { label: '교육과정 수료인원', guide: '수료자 1명 = 1명 인정', weight: 1 }
    ];
  }
