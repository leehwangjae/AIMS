  ...
  target.innerHTML = `
    <section class="dashboard-hero sc">
      <div class="scb">
        ...
        <h2 class="page-title">AI 기안 생성</h2>
        <p class="page-desc">입력정보, KPI, 예산현황을 바탕으로 사업계획서·결과보고서·회의자료 초안을 생성합니다.</p>
      </div>
    </section>
    ${createCard({ title: 'AI 기안 조건 입력', content: renderForm() })}
    ${createCard({ title: 'AI 생성 초안', content: `<pre id="${OUTPUT_ID}" class="draft-output">입력 후 [AI 기안 생성]을 누르세요. OPENAI_API_KEY가 설정되지 않은 경우 기존 템플릿 방식으로 생성됩니다.</pre>` })}
  `;
...
function renderForm() {
  return `
    <form id="${FORM_ID}" class="form-grid">
      <label class="form-field"><span>문서유형</span><select name="documentType"><option value="사업계획서">사업계획서</option><option value="결과보고서">결과보고서</option><option value="회의자료">회의자료</option><option value="심사 Q&A">심사 Q&A</option><option value="보도자료">보도자료</option><option value="인천RISE센터 문의글">인천RISE센터 문의글</option></select></label>
      <label class="form-field"><span>작성스타일</span><select name="writingStyle"><option value="교육부 평가 대응형">교육부 평가 대응형</option><option value="인천시 보고형">인천시 보고형</option><option value="내부 결재형">내부 결재형</option><option value="대외 홍보형">대외 홍보형</option><option value="간략 개조식">간략 개조식</option><option value="상세 서술식">상세 서술식</option></select></label>
      <label class="form-field"><span>분량</span><select name="volume"><option value="1페이지 내외">1페이지 내외</option><option value="3페이지 내외">3페이지 내외</option><option value="5페이지 내외">5페이지 내외</option><option value="상세">상세</option></select></label>
      ...
      <div class="form-actions"><button class="btn btn-primary" type="submit">AI 기안 생성</button><button class="btn btn-outline" type="button" id="copyPlanDraft">초안 복사</button></div>
    </form>`;
}
...
function bindPlanDraftForm() {
  ...
  form.addEventListener('submit', async event => {
    ...
    output.textContent = 'AI 기안 생성 중입니다. OPENAI_API_KEY가 없거나 API 호출에 실패하면 템플릿 초안으로 자동 전환됩니다...';
    const generatedDraft = await generateDraftWithAi(values, selectedBudget);
    output.textContent = generatedDraft;
    ...
    showToast('AI 기안 생성 및 연계 데이터 반영 완료');
  });
...
async function generateDraftWithAi(values, selectedBudget) {
  const context = buildGenerationContext(values, selectedBudget);
  try {
    ...
    return `${buildDraft(values, selectedBudget)}\n\n[참고]\nAI API가 아직 연결되지 않았거나 호출에 실패하여 템플릿 기반 초안을 표시했습니다. Vercel 환경변수 OPENAI_API_KEY를 설정하면 실제 LLM 기반 문서 생성으로 전환됩니다.`;
  } catch (error) {
    return `${buildDraft(values, selectedBudget)}\n\n[참고]\nAI API 호출 중 오류가 발생하여 템플릿 기반 초안을 표시했습니다. 오류: ${error?.message || '알 수 없음'}`;
  }
}

function buildGenerationContext(values, selectedBudget) {
  ...
  return { remainingText, budgetName, afterRemainingText, budgetStatusText, kpiGuide: guideText, kpiUnit, kpiTarget: target, expectedText, contributionText };
}
