  ...
  target.innerHTML = `
    <section class="dashboard-hero sc">
      <div class="scb">
        ...
        <p class="page-desc">입사일 기준 월 만근 연차와 1년 도달 시 80% 이상 출근 여부에 따른 연차를 관리합니다.</p>
      </div>
    </section>
...
function renderUseForm() {
  ...
  return `
    <form id="${USE_FORM_ID}" class="form-grid">
      ...
      <label class="form-field"><span>사유</span><input name="memo" type="text" placeholder="예: 연차휴가" /></label>
...
function calculateAccruedLeave(staff) {
  ...
  const annualLeave = basisDate >= oneYearDate && staff.attendance80 === 'Y' ? 15 : 0;
