  ...
  target.innerHTML = `
    <section class="dashboard-hero sc">
      <div class="scb">
        ...
        <p class="page-desc">프로그램, 참여교원, 참여기업, 증빙자료 첨부 여부를 관리합니다.</p>
      </div>
    </section>

    ${createCard({ title: '졸업생 명단 관리', content: renderGraduateGuide(unitTaskId) })}
    ${createCard({ title: '프로그램 등록', content: renderProgramForm(unitTaskId) })}
    ${createCard({ title: '프로그램 목록 및 증빙상태', content: `<div id="${TABLE_ID}"></div>` })}
  `;
  ...
  if (!hasGraduateKpi) {
    return `
      <div class="evidence-panel">
        <div class="evidence-card"><strong>프로그램 기반 성과관리</strong><p>프로그램 운영실적과 증빙자료를 중심으로 성과를 집계합니다.</p></div>
      </div>
    `;
  }
...
function bindProgramForm(unitTaskId) {
  ...
  form.addEventListener('submit', event => {
    ...
    if (!participantsCheck.valid || !budgetCheck.valid) return showToast('숫자 입력값을 확인해 주세요.');
    ...
    upsertItem('programs', {
      ...
      hasPlan: values.hasPlan,
      hasResultReport: values.hasResultReport,
...
function renderProgramTable(unitTaskId) {
  ...
  const programs = getCollection('programs').filter(program => program.unitTaskId === unitTaskId);

  if (!programs.length) {
    target.innerHTML = createEmptyState({ title: '프로그램 없음', description: '등록된 프로그램이 없습니다.' });
    return;
  }
  ...
  target.innerHTML = `
    ${createTable({
      ...
      rows: programs.map(program => ({ ...program, hasPlan: program.hasPlan === 'Y' ? '첨부완료' : '미첨부', hasResultReport: program.hasResultReport === 'Y' ? '첨부완료' : '미첨부' }))
    })}
  ...
  document.querySelector('#deleteLatestProgram')?.addEventListener('click', () => {
    const latest = getCollection('programs').filter(program => program.unitTaskId === unitTaskId).at(-1);
    if (!latest) return;
